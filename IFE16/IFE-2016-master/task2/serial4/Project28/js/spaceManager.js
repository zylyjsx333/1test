        ...
        sendMessage: function(message) {
            setTimeout(function() {
                ...
                if (decoder.message.command == 'create') {
                    ...
                    commander.commandCenter.message(message);
                }
            }, 300);
        },
        ...
        Adapter: {
            encode: function(receiver, message) {
                ...
                switch(receiver.toString()) {
                    ...
                    case 'captain': returnMsg += '111'; break;
                }
                switch(message.command) {
                    ...
                    case 'broadcast': 
                        returnMsg += ("0" + message.id.toString(2)).substr(-2);
                        returnMsg += message.status;
                        returnMsg += ("000000" + message.energy.toString(2)).substr(-7);
                        returnMsg += ("0" + message.driverModel.toString(2)).substr(-2);
                        returnMsg += ("0" + message.energyModel.toString(2)).substr(-2);
                        break;
                }
                return returnMsg;
            },
            ...
            decode: function(code) {
                ...
                switch(code.substr(0, 3)) {
                    ...
                    case '111': returnMsg.receiver = '星球基地'; break;
                }
                if (returnMsg.receiver == '星球基地') {
                    returnMsg.message.command = 'broadcast';
                    returnMsg.message.id = parseInt(code.substr(3, 2), 2);
                    returnMsg.message.status = parseInt(code.substr(5, 1), 2);
                    returnMsg.message.energy = parseInt(code.substr(6, 7), 2);
                    returnMsg.message.driverModel = parseInt(code.substr(13, 2), 2);
                    returnMsg.message.energyModel = parseInt(code.substr(15, 2), 2);
                } else {
...
(function() {
    spaceManager.record.spaceShipStatusManager = setInterval(function() {
        for(var i = 0; i < spaceManager.record.spaceShipList.length; i++) {
            if(spaceManager.record.spaceShipList[i]._destroyed) {
                continue;
            }
            spaceManager.record.spaceShipList[i].telegraph.sendStatus();
        }
    }, 1000);
})();